...
import verdict7 as v7
import re

app = FastAPI(title="Verdict Cognitive Core API", version="0.7")
...
def infer(req: InferRequest):
    kb = v7.KnowledgeBase()
    try:
        v7.parse_file(req.knowledge_base, kb)
    ...
    # Вывод + Абдукция
    res = v7.evaluate_predicate(kb, pred, args, {})
    if res is None:
        v7.abduce(kb, pred, args)
        res = v7.evaluate_predicate(kb, pred, args, {})
        
    if res:
        val, reason = res
        return {
            "status": v7.TRIT_NAMES[val],
